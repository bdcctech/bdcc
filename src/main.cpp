...
const string strMessageMagic = "BDCC Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
    ...
    if (nVersion == ANON_TXN_VERSION)
    {
        ...
        {
            LogPrintf("CheckTransaction() failed - anon txn in live before block 257000, %s.\n", GetHash().ToString().c_str());
            return false;
        };
    ...
    if (IsCoinBase())
    {
        ...
        else if (vin[0].scriptSig.size() < 2 || vin[0].scriptSig.size() > 300)
            return DoS(100, error("CTransaction::CheckTransaction() : coinbase script size is invalid"));
    } else
    {
        BOOST_FOREACH(const CTxIn& txin, vin)
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock, uint256& hash)
{
    ...
    if (!fReindexing && pblock->IsProofOfStake() && setStakeSeen.count(pblock->GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash))
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        LogPrintf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Bdcc", CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    };
    return true;
