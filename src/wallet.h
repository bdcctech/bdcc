...
public:
    ...
    int64_t GetBdccDebit(const CTxIn& txin) const;
    int64_t GetBdccCredit(const CTxOut& txout) const;
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nBDCC, int64_t& nBdcc) const
    {
        nBDCC = 0;
        nBdcc = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nBdcc += GetBdccCredit(txout);
            } else
                nBDCC += GetCredit(txout);
            if (!MoneyRange(nBDCC)
                || !MoneyRange(nBdcc))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
...
public:
    ...
    mutable bool fAvailableBdccCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableBdccCreditCached;
    
    mutable int64_t nCredBDCCCached;
    mutable int64_t nCredBdccCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableBdccCreditCached = false;
    ...
    bool GetCredit(int64_t& nCredBDCC, int64_t& nCredBdcc, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredBDCC = nCredBdcc = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredBDCCCached = 0;
            nCredBdccCached = 0;
            pwallet->GetCredit(*this, nCredBDCCCached, nCredBdccCached);
            fCreditSplitCached = true;
        };
        
        nCredBDCC = nCredBDCCCached;
        nCredBdcc = nCredBdccCached;
        return true;
    }
    ...
    int64_t GetAvailableBdccCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableBdccCreditCached)
            return nAvailableBdccCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableBdccCredit() : value out of range");
            };
        };
        
        nAvailableBdccCreditCached = nCredit;
        fAvailableBdccCreditCached = true;
        return nCredit;
    };
