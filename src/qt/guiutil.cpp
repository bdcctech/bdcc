...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bdccurrency"))
        return false;
...
{
    // Convert bdccurrency:// to bdccurrency:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bdccurrency://"))
    {
        uri.replace(0, 12, "bdccurrency:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BDCCurrency.lnk";
}
...
{
    return GetAutostartDir() / "bdccurrency.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BDCCurrency-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bdccurrency-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BDCCurrency-Qt"));
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(190,135,44); color: rgb(21,21,21); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(190,135,44); border-left-style: solid; border-left-width: 6px; }"
