...
var qrcode = new QRCode("qrcode", {colorDark:'#131218', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H, width: 220, height: 220,});
...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uBDCC",
                this.display = "&micro;BDCC";
                break;

            case 3:
                this.name    = "sBDCC",
                this.display = "BDCCtoshi";
                break;

            default:
                this.name = this.display = "BDCC";
        }
...
var overviewPage = {
    init: function() {
        ...
        // Announcement feed
        $.ajax({
            url:"https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&q=http://updates.bdcc.co/rss",
            dataType: 'jsonp'
    ...
    updateBalance: function(balance, bdccBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            bdccBal   = this.bdccBal  .data("orig"),
            ...
            this.bdccBal  .data("orig", bdccBal),
        ...
        this.formatValue("bdccBal",   bdccBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. BkmGw594H2F1cMWwFu9hQNkbUWPZeBvJaE)" placeholder="Enter an BDCC address (e.g. BkmGw594H2F1cMWwFu9hQNkbUWPZeBvJaE)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="Bdcc"                    ' + (unit.type == 0 ? "selected" : "") + '>BDCC</option> \
                    <option value="1" title="Milli-Bdcc (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mBDCC</option> \
                    <option value="2" title="Micro-Bdcc (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;BDCC</option> \
                    <option value="3" title="BDCCtoshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>BDCCtoshi</option> \
                </select> \
            </div> \
