...
#include "bdccbridge.moc"

BdccBridge::BdccBridge(BdccGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void BdccBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void BdccBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void BdccBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void BdccBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void BdccBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void BdccBridge::populateOptions()
{
...
// Transactions
bool BdccBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void BdccBridge::clearRecipients()
{
    recipients.clear();
}

bool BdccBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 BDCC, 1 BDCC
        switch(rcp.txnTypeInd)
        {
            case TXT_BDCC_TO_BDCC:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BDCC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_BDCC_TO_ANON:
                formatted.append(tr("<b>%1</b> to BDCC %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BDCC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to BDCC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BDCC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_BDCC:
                formatted.append(tr("<b>%1</b>, ring size %2 to BDCC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BDCC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BDCC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsBdccBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your bdcc balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BDCC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void BdccBridge::openAddressBook(bool sending)
{
...
void BdccBridge::openCoinControl()
{
...
void BdccBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void BdccBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap BdccBridge::listAnonOutputs()
{
...
void BdccBridge::populateTransactionTable()
{
...
void BdccBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void BdccBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString BdccBridge::transactionDetails(QString txid)
{
...
// Addresses
void BdccBridge::populateAddressTable()
{
...
void BdccBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void BdccBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString BdccBridge::newAddress(bool own)
{
...
QString BdccBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void BdccBridge::updateAddressLabel(QString address, QString label)
{
...
bool BdccBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool BdccBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void BdccBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void BdccBridge::appendMessage(int row) {
...
void BdccBridge::populateMessageTable()
{
...
void BdccBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool BdccBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool BdccBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString BdccBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool BdccBridge::setPubKey(QString address, QString pubkey)
{
...
bool BdccBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void BdccBridge::connectSignals()
{
...
QVariantMap BdccBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap BdccBridge::listLatestBlocks()
{
...
QVariantMap BdccBridge::findBlock(QString searchID)
{
...
QVariantMap BdccBridge::blockDetails(QString blkHash)
{
...
QVariantMap BdccBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap BdccBridge::txnDetails(QString blkHash, QString txnHash)
{
