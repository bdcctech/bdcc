...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return BdccGUI::tr("Received with");
    case RecvFromOther:
        return BdccGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return BdccGUI::tr("Sent to");
    case SendToSelf:
        return BdccGUI::tr("Payment to yourself");
    case Generated:
        return BdccGUI::tr("Mined");
    case RecvBdcc:
        return BdccGUI::tr("Received bdcc");
    case SendBdcc:
        return BdccGUI::tr("Sent bdcc");
    case Other:
        return BdccGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvBdcc:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendBdcc:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredBDCC, nCredBdcc;
    wtx.GetCredit(nCredBDCC, nCredBdcc, true);
    int64_t nCredit = nCredBDCC + nCredBdcc;
    ...
    {
        if (nNet > 0 && nCredBdcc > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvBdcc, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendBdcc, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvBdcc;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv bdcc
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendBdcc;
                    sub.address = CBitcoinAddress(ckidD).ToString();
