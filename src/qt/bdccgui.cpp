// Copyright (c) 2014 The BDCC Developers
...
BdccGUI::BdccGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new BdccBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
...
#ifdef Q_OS_WIN
    QFile html("C:/bdcc/index.html");
#else
    QFile html("/opt/bdcc/index.html");
#endif
...
void BdccGUI::pageLoaded(bool ok)
{
...
void BdccGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void BdccGUI::urlClicked(const QUrl & link)
{
...
void BdccGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("BDCC") + " - " + tr("Wallet") + ", " + sMode);
        };
        ...
        if (clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Bdcc client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/bdcc_testnet"));
...
void BdccGUI::setWalletModel(WalletModel *walletModel)
{
...
void BdccGUI::setMessageModel(MessageModel *messageModel)
{
...
void BdccGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Bdcc client"));
    trayIcon->setIcon(QIcon(":/icons/bdcc"));
...
#ifndef Q_OS_MAC
void BdccGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BdccGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to Bdcc network", "", count));
}

void BdccGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void BdccGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void BdccGUI::changeEvent(QEvent *e)
{
...
void BdccGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void BdccGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::BDCC, nFeeRequired));
...
void BdccGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void BdccGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void BdccGUI::gotoSignMessageTab(QString addr)
{
...
void BdccGUI::gotoVerifyMessageTab(QString addr)
{
...
void BdccGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void BdccGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BdccGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void BdccGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Bdcc address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void BdccGUI::handleURI(QString strURI)
{
...
void BdccGUI::setEncryptionStatus(int status)
{
...
void BdccGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void BdccGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void BdccGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BdccGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void BdccGUI::updateStakingIcon()
{
