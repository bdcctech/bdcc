...
void ThreadIRCSeed()
{
    ...
    for(;;)
    {
        ...
        if (fTestNet)
        {
            Send(hSocket, "JOIN #bdccTEST\r");
            Send(hSocket, "WHO #bdccTEST\r");
        } else
        {
            // randomly join #bdcc00-#bdcc05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #bdcc%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bdcc%02d\r", channel_number).c_str());
        };
